...
(function () {
  ...
  // 按钮样式常量
  const BUTTON_CLASS = "whitespace-nowrap font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed px-3 py-1 text-xs rounded-md bgm-bg text-white font-semibold shadow-sm hover:opacity-90";
  ...
  const addButton = (containerSelector, buttonId, buttonText, onClick) => {
    ...
    const $button = $(`<button id="${buttonId}" class="${BUTTON_CLASS}">${buttonText}</button>`);
    $button.on("click", onClick);
    $container.append($button);
  };

  // 工具函数：为所有匹配的容器添加按钮（防重复）
  const addButtonToAll = (containerSelector, buttonClass, buttonText, onClick) => {
    $(containerSelector).each(function () {
      const $container = $(this);
      if ($container.find(`.${buttonClass}`).length > 0) return;

      const $button = $(`<button class="${buttonClass} ${BUTTON_CLASS}">${buttonText}</button>`);
      $button.on("click", onClick);
      $container.append($button);
    });
  };
  ...
  if (pathName.startsWith("/rakuen/home")) {
    waitForElement("#tg-rakuen-home-user-card #tg-rakuen-home-user-actions", () => {
      addButton(
        ...
        function () {
          $.ajax({
            ...
            error: (xhr) => {
              ...
              const start = text.indexOf(".AspNetCore.Identity.Application=") + 33;
              const end = text.indexOf("Origin:");
              const cookie = text.substring(start, end);

              if (confirm("是否同意将小圣杯操作权限授权给fuyuake，每次小圣杯退出登录或重新登录都会使授权失效")) {
    ...
    waitForElement(
      ...
      () => {
        addButtonToAll(
          ...
          "FUYUAKE",
          function () {
            const charaId = $(this).closest("#tg-trade-box").attr("data-character-id");
            if (charaId) {
